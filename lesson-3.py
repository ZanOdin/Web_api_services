...
while True:
    ...
    for vacancy in vacancies:
        vacancy_data = {}
        salary = []
        name_web = vacancy.find('div', {'class': 'vacancy-serp-item__info'}).getText()
        employer = vacancy.find('div', {'class': 'vacancy-serp-item__meta-info-company'}).getText()
        employer_web = employer.replace(u'\xa0', " ")
        try:
            web_salary = vacancy.find('div', {'class': 'vacancy-serp-item__sidebar'}).getText()
        except:
            web_salary = None
        salary.append(take_salary(web_salary))
        vacancy_ref = vacancy.find('a', {'class': 'bloko-link'})['href']
        vacancy_data['name'] = name_web
        vacancy_data['salary'] = salary
        vacancy_data['employer'] = employer_web
        vacancy_data['info'] = vacancy_ref
        vacancy_data['site'] = response.url
        vacancies_list.append(vacancy_data)

        # ------------------------------------- Новый код -----------------------------------
        result = vacancies_mongo.find_one({'name': name_web, 'employer': employer_web})
        if result is None:
            vacancies_mongo.insert_one(vacancy_data)

    page += 1
print("Результаты добавлены в базу данных vacancies")
print(f"Количество вакансий: {vacancies_mongo.estimated_document_count()}")
